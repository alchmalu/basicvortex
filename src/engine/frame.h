...
namespace vortex {
  ...
  public:
    ...
    void setPositionWithConstraint(glm::vec3& position);
    ...
    glm::vec3 position() const { return inverseCoordinatesOf(glm::vec3(0.0,0.0,0.0)); }
  ...
  //@}
    ...
    void setTranslation(const glm::vec3& translation) { t_ = translation; /*Q_EMIT modified();*/ }
    ...
    void rotateAroundPoint(Quaternion& rotation, const glm::vec3& point);
  ...
  public:
    ...
    void getWorldMatrix(double m[4][4]) const;
    void getWorldMatrix(double m[16]) const;

    void setFromMatrix(const double m[4][4]);
    void setFromMatrix(const double m[16]);
    //@}

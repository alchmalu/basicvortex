    ...
    mSkeleton(NULL)
{
    ...
    for (unsigned int i = 0; i < mNumChannels; ++i) {
        ...
        for (unsigned int j = 0; j < boneAnim->mNumRotationKeys; ++j) {
            ...
            // Convertion aiQuatKey to local Quaternion representation

            Quaternion quat(aiAnim->mRotationKeys[j].mValue.x,
                            aiAnim->mRotationKeys[j].mValue.y,
                            aiAnim->mRotationKeys[j].mValue.z,
                            aiAnim->mRotationKeys[j].mValue.w);

            boneAnim->mRotationKeys[j] = new QuadKey(aiAnim->mRotationKeys[j].mTime / mTicksPerSecond, quat);
        }
...
#define TIME
    ...
    Quaternion q = Quaternion::slerp(q0, q1, 1.-timeRemains);

    glm::mat3x3 kRotation;
    q.getRotationMatrix(kRotation);
    kRotation = glm::transpose(kRotation);
    ...
    glm::vec3 p = timeRemains*p0 + (1.0f-timeRemains)*p1; // channel->mPositionKeys[frame]->mValue
    //p=p0;
